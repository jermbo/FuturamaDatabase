...
var CharacterDetail = Vue.extend({
  template: '#character-details',
  data: function() {
    console.log('in char details')
    var character = this.$parent.characters.filter(char => char.id == this.$route.params.id)
...
var PersonDetail = Vue.extend({
  template: '#people-detail',
  data: function() {
    console.log('waht the fuck')
    var person;
    for (var i = 0; i < this.$parent.people.length; i++) {
      if (this.$parent.people[i].id == this.$route.params.id) {
